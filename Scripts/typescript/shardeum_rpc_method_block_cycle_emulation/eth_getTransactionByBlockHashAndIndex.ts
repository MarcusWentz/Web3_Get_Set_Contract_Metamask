...
async function eth_getTransactionByHashNumberAndIndex(blockHash: string, txIndex: number){
  let cycleData = await eth_getBlockByHash(
    "0xf2a9a9aa1e71d24f0f072f7ee3a2a46df1dd5b30ecdf44aa4742372470ebd1a4"
  );
  let cycleNumber = cycleData.cycles[0].counter;
  let arrayTransactionsInOrder =  await eth_getBlockReceipts(cycleNumber);
  return arrayTransactionsInOrder[txIndex];
}

async function test_eth_getTransactionByBlockNumberAndIndex() {

  let txIndex =  await eth_getTransactionByHashNumberAndIndex(
    "0xf2a9a9aa1e71d24f0f072f7ee3a2a46df1dd5b30ecdf44aa4742372470ebd1a4", //Cycle 2000
    8 // Transaction 9/9 (index 8 we are counting from 0).
  ); 
